 ...
 * -> Create a channel 
 * -> Bind the socket
 * associated with this channel to a <client-port> 
 * -> Configure the channel as
 * non-blocking 
 * -> Register the channel with selector. 
 * -> Invoke select() so
 * that it blocks until registered channel is ready. (as opposed to select(long timeout) 
 * -> Get the set of keys whose underlying channel is ready for the
 * operation they showed interest when they registered with the selector. 
 * ->
 * Iterate through the keys. 
 * -> For every key check if the underlying channel is
 * ready for the operation it is interested in. 
 * -> If ready print message of
 * readiness.
 * 
 * Notes: 
 * -> Need MultiJabberServer running on the local machine. You run it to
 * connect to the local MultiJabberServer 
 * -> It causes and exception at
