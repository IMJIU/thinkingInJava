...
/**
 * Customer  waitPerson.placeOrder(this, food); restaurant.orders.put(Order(cust, this, food)); 
 *  -> Chef restaurant.orders.take();  order.getWaitPerson().filledOrders.put(plate);
 *  -> Waiter  filledOrders.take(); order.getWaitPerson().filledOrders.put(plate);
 *  -> Customer placeSetting.take();
 *  BlockingQueue<Order> orders = new LinkedBlockingQueue<Order>();
 *  BlockingQueue<Plate> filledOrders = new LinkedBlockingQueue<Plate>();
 *  SynchronousQueue<Plate> placeSetting = new SynchronousQueue<Plate>();// Only one course at a time can be received:
 * @author root
 *
 */
